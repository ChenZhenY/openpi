...
class Args:
    ...
    resize_size: int = 224
    action_horizon: int = (
        10  # Action horizon for ActionChunkBroker (RTC) or action chunk size (non-RTC)
    )
    replan_steps: int = (
        5  # When use_rtc=False, replan every N steps (similar to main.py)
    )
    latency_ms: float = (
    ...
    #################################################################################################################
    video_out_path: str = (
        "examples/libero/data/moving_bowl/videos"  # Path to save videos
    )
    results_csv_path: str = (
        "examples/libero/data/moving_bowl/results.csv"  # Path to save CSV results
    )
...
def find_bowl_state_index(env, verbose=False):
    ...
    if verbose:
        ...
        if len(state_array) > bowl_start_idx + 6:
            logging.info(
                f"Bowl XYZ: {state_array[bowl_start_idx : bowl_start_idx + 3]}"
            )
            logging.info(
                f"Bowl quaternion (qw,qx,qy,qz): {state_array[bowl_start_idx + 3 : bowl_start_idx + 7]}"
            )
...
def is_bowl_moved_from_start(
    env, bowl_idx_start=9, initial_bowl_xy=None, distance_threshold=0.05
):
    """
...
def move_bowl_if_needed(
    ...
    bowl_relocated = is_bowl_moved_from_start(
        env,
        bowl_idx_start=9,
        initial_bowl_xy=initial_bowl_xy,
    ...
    if bowl_grasped and log_grasped and sim_step < 200:
        ...
        logging.info(
            f"Episode {episode_idx}, sim_step {sim_step}: Skipping bowl movement (bowl is lifted)"
        )
    ...
    if bowl_relocated and episode_idx == 0 and not bowl_relocated_logged:
        ...
        logging.info(
            f"Episode {episode_idx}, sim_step {sim_step}: Bowl has been moved from start position, stopping movement"
        )
...
def eval_moving_bowl(
    ...
    if args.use_rtc:
        ...
        broker = action_chunk_broker.ActionChunkBroker(
            policy=policy_client, **action_chunk_broker_params
        )
        broker.reset()
        action_plan = None
    else:
    ...
    while t < args.max_steps + args.num_steps_wait:
        try:
            ...
            if INITIAL_BOWL_Z is None:
                ...
                if episode_idx == 0:
                    logging.info(
                        f"Initial bowl position (after stabilization): XY={INITIAL_BOWL_XY}, Z={INITIAL_BOWL_Z}"
                    )
                    logging.info(
                        f"Bowl will be considered grasped if Z > {INITIAL_BOWL_Z + 0.02}"
                    )
                    logging.info(f"Bowl movement range: Y in [{MIN_Y}, {MAX_Y}]")
                    logging.info(
                        f"Bowl movement will stop if moved > {RELOCATION_THRESHOLD:.3f}m from start position"
                    )
                    logging.info(
                        "Bowl movement based on sim_step (independent of artificial latency)"
                    )
    ...
    video_filename = (
        video_out_path
        / f"ep{episode_idx}_rtc{args.use_rtc}_hrzn{args.action_horizon}_{suffix}.mp4"
    )
    imageio.mimwrite(
        video_filename,
    ...
    logging.info(
        f"Episode {episode_idx}: {'SUCCESS' if success else 'FAIL'} - {t} steps, reward: {total_reward:.3f}"
    )
...
def load_initial_states(init_file_path: str):
    ...
    try:
        ...
        initial_states = torch.load(
            str(init_file_path), map_location="cpu", weights_only=False
        )
    except TypeError:
...
def main(args: Args) -> None:
    ...
    # Create environment
    env, task_description = create_libero_env_from_bddl(
        args.bddl_file, LIBERO_ENV_RESOLUTION, args.seed
    )

    # Log output paths
    logging.info("=" * 80)
    logging.info("Moving Bowl Evaluation - LIBERO KITCHEN_SCENE1")
    logging.info("=" * 80)
    ...
    if args.use_rtc:
        ...
        logging.info(
            f"Mode: RTC disabled, action horizon: {args.action_horizon}, replan every {args.replan_steps} steps"
        )
    logging.info(
        f"Object movement: every {args.n_obj_steps} steps, step size: {args.obj_step}"
    )
    ...
    if args.num_trials > len(initial_states_list):
        logging.warning(
            f"Requested {args.num_trials} trials but only {len(initial_states_list)} initial states available. Using {len(initial_states_list)} trials."
        )
    ...
    logging.info(
        f"Success rate: {total_successes}/{args.num_trials} ({success_rate * 100:.1f}%)"
    )
    logging.info("=" * 80)
    ...
    if all_results:
        ...
        summary = {
            ...
            "avg_inference_time": np.mean(
                [r["avg_inference_time"] for r in all_results]
            ),
            "avg_additional_delay_time": np.mean(
                [r["avg_additional_delay_time"] for r in all_results]
            ),
