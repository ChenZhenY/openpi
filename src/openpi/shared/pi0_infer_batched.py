    ...
    for step in range(10):
        ...
        for i in range(18):
            ...
            for b in range(batch_size):
                ...
                matmul_k8_256_n_softmax_mask0(
                    decoder_q_buf_b,
                    buffers['encoder_K'][b, i],
                    decoder_attn_buf_b,
                    encoder_seq_len
                )
                matmul_k8_n_256(
                    decoder_attn_buf_b,
                    buffers['encoder_V'][b, i],
                    decoder_q_buf_b
                )
                
                matmul_k_2048_1024_res(
                    decoder_q_buf_b.view(-1, 2048),
                    weights['decoder_attn_o_w'][i],
                    decoder_x_b
                )
...
class Pi0InferenceBatched:
    ...
    def __init__(self, checkpoint, num_views, chunk_size, batch_size):
        ...
        self.buffers = {
            'observation_images_normalized':   torch.empty(batch_size, num_views, 224, 224, 3,  dtype=torch.bfloat16, device="cuda"),
            ...
            'encoder_K':                       torch.empty(batch_size, 18, encoder_seq_len + decoder_seq_len, 256, dtype=torch.bfloat16, device="cuda"),
            'encoder_V':                       torch.empty(batch_size, 18, encoder_seq_len + decoder_seq_len, 256, dtype=torch.bfloat16, device="cuda"),
            ...
            'decoder_x_actions':               torch.empty(batch_size, chunk_size, 1024,        dtype=torch.bfloat16, device="cuda"),  # Separate buffer for action tokens
            ...
            'decoder_norm_factor_actions':     torch.empty(batch_size, chunk_size,              dtype=torch.bfloat16, device="cuda"),  # Separate buffer for action norm factors
            'decoder_q_buf':                   torch.empty(batch_size, decoder_seq_len * 8, 256, dtype=torch.bfloat16, device="cuda"),
            'decoder_attn_buf':                torch.empty(batch_size, decoder_seq_len * 8, encoder_seq_len + decoder_seq_len, dtype=torch.bfloat16, device="cuda"),
