...
def merge_split_k_bias_res(
    ...
    for i in range(pid * BLOCK, seq_len * hidden, psize * BLOCK):
        ...
        mask = (i + tl.arange(0, BLOCK)) < (seq_len * hidden)
        tl.store(final_out_ptr + i + tl.arange(0, BLOCK), acc.to(TL_DTYPE), mask=mask)
...
def combine_1536_1152_twopart(out_ptr, inp_ptr, seq_len: tl.constexpr, hidden: tl.constexpr):
    ...
    for i in range(pid * 2, seq_len, psize * 2):
        ...
        tl.store(out_ptr + (i + tl.arange(0, 2)[:, None]) * hidden + 1024 + tl.arange(0, 128), inp.to(TL_DTYPE))
...
def vision_encoder(weights, buffers, num_views, batch_size):
    ...
    for i in range(27):
        layer_norm_QKV_matmul_n256_1152_3456_bias(
            ...
            vision_QKV_reshaped,
            vision_x_norm_reshaped,
        )

        attn = AttnMultiKey(vision_QKV_reshaped)
...
def transformer_encoder(weights, buffers, encoder_seq_len, batch_size):
    ...
    encoder_hidden_reshaped = buffers["encoder_hidden"].view(batch_size * encoder_seq_len, 16384)
    ...
    for i in range(18):
        # Process each batch separately only for attention due to KV cache indexing
        ...
        if i != 17:
            # FFN can be batched across all examples
            rms_matmul_n_2048_16384_gate(
                encoder_x_reshaped,
                weights["encoder_ffn_gate_w"][i],
                weights["encoder_ffn_up_w"][i],
                encoder_hidden_reshaped,
                encoder_x_norm_reshaped,
            )

            matmul_n_16384_2048_res(encoder_hidden_reshaped, weights["encoder_ffn_down_w"][i], encoder_x_reshaped)
...
def matvec_bias_kernel(x_ptr, weight_ptr, bias_ptr, out_ptr, features: tl.constexpr, hidden: tl.constexpr):
    ...
    for j in range(pid * BLOCK_SIZE_M, hidden, psize * BLOCK_SIZE_M):
        ...
        tl.store(
            out_ptr + j + tl.arange(0, BLOCK_SIZE_M), acc.to(TL_DTYPE), mask=j + tl.arange(0, BLOCK_SIZE_M) < hidden
        )
...
def scaled_matmul_rope_qkv(
    ...
    while pid < grid_m * grid_n:
        ...
        accumulator = accumulator.to(TL_DTYPE)
...
def transformer_decoder(weights, buffers, encoder_seq_len, batch_size):
    # Reshape buffers for batch processing
    decoder_seq_len = buffers["decoder_x"].shape[1]
    decoder_x_reshaped = buffers["decoder_x"].view(batch_size * decoder_seq_len, 1024)
    decoder_x_buf_reshaped = buffers["decoder_x_buf"].view(batch_size * decoder_seq_len, 1024)
    decoder_norm_factor_buf_reshaped = buffers["decoder_norm_factor_buf"].view(batch_size * decoder_seq_len)
    decoder_hidden_reshaped = buffers["decoder_hidden"].view(batch_size * decoder_seq_len, 4096)
    diffusion_noise_reshaped = buffers["diffusion_noise"].view(batch_size * buffers["diffusion_noise"].shape[1], 32)

    # State initialization - process each batch separately due to single token
    for b in range(batch_size):
        matmul_1_32_1024_bias(
            ...
            buffers["decoder_x"][b, :1],
        )

    for step in range(10):
        # These can be batched across all examples
        matmul_k_32_1024_bias_silu(
            diffusion_noise_reshaped,
            weights["decoder_action_fused_in_proj_w"],
            weights["decoder_action_fused_time_biases"][step % 10],
            decoder_x_buf_reshaped,
        )
        matmul_k_1024_1024_bias(
            decoder_x_buf_reshaped,
            weights["decoder_action_mlp_w"],
            weights["decoder_action_mlp_b"],
            decoder_x_reshaped[batch_size:],  # Skip first token of each batch
        )

        for i in range(18):
            # Attention must be done per batch due to KV cache indexing
            for b in range(batch_size):
                decoder_x_b = buffers["decoder_x"][b]
                decoder_norm_factor_buf_b = buffers["decoder_norm_factor_buf"][b]
            ...
            rms_matmul_k_1024_4096_gate(
                ...
                decoder_hidden_reshaped,
                decoder_norm_factor_buf_reshaped,
            )
            matmul_k_4096_1024_res(decoder_hidden_reshaped, weights["decoder_ffn_down_w"][i], decoder_x_reshaped)

        # Final projection can be batched
        rms_matmul_k_1024_32_bias_res(
            decoder_x_reshaped[batch_size:],  # Skip first token of each batch
            weights["decoder_action_fused_out_proj_w"],
            weights["decoder_action_fused_out_proj_b"],
            diffusion_noise_reshaped,
            decoder_norm_factor_buf_reshaped[batch_size:],
        )


def pi0_model(weights, buffers, num_views, batch_size):
    encoder_seq_len = buffers["encoder_x"].shape[1]  # Shape is now (batch_size, encoder_seq_len, ...)
    vision_encoder(weights, buffers, num_views, batch_size)
    transformer_encoder(weights, buffers, encoder_seq_len, batch_size)
    transformer_decoder(weights, buffers, encoder_seq_len, batch_size)


class Pi0Inference:
    def __init__(self, checkpoint, num_views, chunk_size, batch_size=1):
        ...
        self.weights = {
            "vision_patch_embedding_w": torch.empty(14, 14, 3, 1152, dtype=TORCH_DTYPE, device="cuda"),
            "vision_patch_embedding_b": torch.empty(1152, dtype=TORCH_DTYPE, device="cuda"),
            "vision_position_embedding": torch.empty(256, 1152, dtype=TORCH_DTYPE, device="cuda"),
            "vision_attn_qkv_w": torch.empty(27, 1152, 3 * 1152, dtype=TORCH_DTYPE, device="cuda"),
            "vision_attn_qkv_b": torch.empty(27, 3 * 1152, dtype=TORCH_DTYPE, device="cuda"),
            "vision_attn_o_w": torch.empty(27, 1152, 1152, dtype=TORCH_DTYPE, device="cuda"),
            "vision_attn_o_b": torch.empty(27, 1152, dtype=TORCH_DTYPE, device="cuda"),
            "vision_ffn_up_w": torch.empty(27, 1152, 4304, dtype=TORCH_DTYPE, device="cuda"),
            "vision_ffn_up_b": torch.empty(27, 4304, dtype=TORCH_DTYPE, device="cuda"),
            "vision_ffn_down_w": torch.empty(27, 4304, 1152, dtype=TORCH_DTYPE, device="cuda"),
            "vision_ffn_down_b": torch.empty(27, 1152, dtype=TORCH_DTYPE, device="cuda"),
            "vision_pre_attn_norm_w": torch.empty(27, 1152, dtype=TORCH_DTYPE, device="cuda"),
            "vision_pre_attn_norm_b": torch.empty(27, 1152, dtype=TORCH_DTYPE, device="cuda"),
            "vision_pre_ffn_norm_w": torch.empty(27, 1152, dtype=TORCH_DTYPE, device="cuda"),
            "vision_pre_ffn_norm_b": torch.empty(27, 1152, dtype=TORCH_DTYPE, device="cuda"),
            "vision_final_norm_w": torch.empty(1152, dtype=TORCH_DTYPE, device="cuda"),
            "vision_final_norm_b": torch.empty(1152, dtype=TORCH_DTYPE, device="cuda"),
            "encoder_multi_modal_projector_w": torch.empty(1152, 2048, dtype=TORCH_DTYPE, device="cuda"),
            "encoder_multi_modal_projector_b": torch.empty(2048, dtype=TORCH_DTYPE, device="cuda"),
            "encoder_attn_qkv_w": torch.empty(18, 2048, 2560, dtype=TORCH_DTYPE, device="cuda"),
            "encoder_attn_o_w": torch.empty(18, 2048, 2048, dtype=TORCH_DTYPE, device="cuda"),
            "encoder_ffn_gate_w": torch.empty(18, 2048, 16384, dtype=TORCH_DTYPE, device="cuda"),
            "encoder_ffn_up_w": torch.empty(18, 2048, 16384, dtype=TORCH_DTYPE, device="cuda"),
            "encoder_ffn_down_w": torch.empty(18, 16384, 2048, dtype=TORCH_DTYPE, device="cuda"),
            "decoder_state_in_proj_w": torch.empty(32, 1024, dtype=TORCH_DTYPE, device="cuda"),
            "decoder_state_in_proj_b": torch.empty(1024, dtype=TORCH_DTYPE, device="cuda"),
            "decoder_action_fused_in_proj_w": torch.empty(32, 1024, dtype=TORCH_DTYPE, device="cuda"),
            "decoder_action_fused_time_biases": torch.empty(10, 1024, dtype=TORCH_DTYPE, device="cuda"),
            "decoder_action_mlp_w": torch.empty(1024, 1024, dtype=TORCH_DTYPE, device="cuda"),
            "decoder_action_mlp_b": torch.empty(1024, dtype=TORCH_DTYPE, device="cuda"),
            "decoder_attn_qkv_w": torch.empty(18, 1024, 2560, dtype=TORCH_DTYPE, device="cuda"),
            "decoder_attn_o_w": torch.empty(18, 2048, 1024, dtype=TORCH_DTYPE, device="cuda"),
            "decoder_ffn_gate_w": torch.empty(18, 1024, 4096, dtype=TORCH_DTYPE, device="cuda"),
            "decoder_ffn_up_w": torch.empty(18, 1024, 4096, dtype=TORCH_DTYPE, device="cuda"),
            "decoder_ffn_down_w": torch.empty(18, 4096, 1024, dtype=TORCH_DTYPE, device="cuda"),
            "decoder_action_fused_out_proj_w": torch.empty(1024, 32, dtype=TORCH_DTYPE, device="cuda"),
            "decoder_action_fused_out_proj_b": torch.empty(32, dtype=TORCH_DTYPE, device="cuda"),
            "language_embeds": torch.empty(self.prompt_len, 2048, dtype=TORCH_DTYPE, device="cuda"),
        }
        ...
        k_cos = torch.cos(k_phase).to(TORCH_DTYPE)
        k_sin = torch.sin(k_phase).to(TORCH_DTYPE)
    ...
    def record_run(self):
        # Copy language embeds to all batch elements
        for b in range(self.batch_size):
            self.buffers["encoder_x"][b, self.num_views * 256 :].copy_(self.weights["language_embeds"])
        pi0_model(self.weights, self.buffers, self.num_views, self.batch_size)
