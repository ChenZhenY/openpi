...
def create_batch_input(batch_size: int, num_views: int, chunk_size: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
...
def benchmark_single(model, num_views: int, chunk_size: int, num_warmup: int = 5, num_iters: int = 20) -> dict[str, Any]:
...
def benchmark_single_loop(model, batch_size: int, num_views: int, chunk_size: int, 
                          num_warmup: int = 5, num_iters: int = 20) -> dict[str, Any]:
...
def benchmark_batched(model, batch_size: int, num_views: int, chunk_size: int,
                      num_warmup: int = 5, num_iters: int = 20) -> dict[str, Any]:
...
def main():
    ...
    from openpi.shared.pi0_infer_batched import Pi0Inference, Pi0InferenceBatched
    

    with open("/srv/rl2-lab/flash8/rbansal66/openpi_rollout/openpi/.cache/openpi/openpi-assets/checkpoints/pi0_libero_pytorch_dexmal_mokapots/converted_checkpoint.pkl", 'rb') as f:
    ...
    single_results = benchmark_single(
        single_model, args.num_views, args.chunk_size,
    ...
    for batch_size in args.batch_sizes:
        ...
        loop_results = benchmark_single_loop(
            single_model, batch_size, args.num_views, args.chunk_size,
            args.num_warmup, args.num_iters
        )
        ...
        try:
            ...
            batched_results = benchmark_batched(
                batched_model, batch_size, args.num_views, args.chunk_size,
                args.num_warmup, args.num_iters
            )
    ...
    print(f"{'Single (baseline)':<30} {all_results['single']['mean_ms']:<15.2f} {all_results['single']['throughput']:<15.2f} {'1.00x':<10}")
    ...
    for batch_size in args.batch_sizes:
        ...
        if loop_key in all_results:
            ...
            print(f"{f'Loop (batch={batch_size})':<30} {all_results[loop_key]['mean_ms']:<15.2f} {all_results[loop_key]['throughput']:<15.2f} {f'{speedup:.2f}x':<10}")
        
        if batched_key in all_results:
            speedup = all_results[batched_key]["throughput"] / baseline_throughput
            print(f"{f'Batched (batch={batch_size})':<30} {all_results[batched_key]['mean_ms']:<15.2f} {all_results[batched_key]['throughput']:<15.2f} {f'{speedup:.2f}x':<10}")
